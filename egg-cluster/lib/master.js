...
class Master extends EventEmitter {
  ...
  constructor(options) {
    ...
    this.workerManager.on("exception", ({ agent, worker }) => {
      const err = new Error(
        `[master] ${agent} agent and ${worker} worker(s) alive, exit to avoid unknown state`
      );
  ...
  close() {
    ...
    this.log(
      "[master] send kill SIGTERM to app workers and agent worker, will exit with code:0 after %sms",
      timeout
    );
